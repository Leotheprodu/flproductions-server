...
const sendAEmail = async  (templateName, dataToEJS, from, to, subject ) => {
    ...
    await ejs.renderFile(__dirname + `/../config/nodemailer/templates/${templateName}.ejs`, dataToEJS, (error, data) => {
        if (error) {
            console.log(error);
            ...
            transporter.sendMail(mailOptions, (error, info) => {
                if (error) {
                    ...
                    const message = { message: `Correo electrónico enviado: ${info.response}` };
