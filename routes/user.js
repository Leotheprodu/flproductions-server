    ...
    connection.query(
        ...
        function (error, results) {
            if (error) {
                console.error(error);
                res.status(500).json({
                    error: 'Ha ocurrido un error al consultar en usuarios',
                });
                return;
            }
            if (results.length === 1) {
                connection.query(
                    ...
                    function (error, results, fields) {
                        if (error) {
                            console.error(error);
                            res.status(500).json({
                                error: 'Ha ocurrido un error al guardar los el registro en temp_token_pool',
                            });
                            return;
                        }
                        ejs.renderFile(
                            __dirname +
                                '/../config/nodemailer/templates/user-recuperar-password.ejs',
                            { token },
                            (error, data) => {
                                if (error) {
                                    console.log(error);
                                                        ...
                                                        function (
                                                            ...
                                                            if (error) {
                                                                ...
                                                                res.status(
                                                                    ...
                                                                    error: 'Ha ocurrido un error al borrar los el registro en temp_token_pool',
                                                                });
                                                                return;
                                                            }
                                                        }
                ...
                res.status(404).json({
                    message: 'El email no esta en el sistema',
                });
            }
        }
    );
...
router.post('/recover-password-step2', (req, res) => {
    ...
    connection.query(
        'SELECT * FROM temp_token_pool WHERE token = ?',
        pin,
        function (err, resultados) {
            if (err) {
                console.error(err);
                res.status(500).json({
                    error: 'Ha ocurrido un error al buscar el PIN',
                });
            ...
            if (resultados.length === 1 && resultados[0].token === pin) {
                                ...
                                bcrypt.hash(
                                    ...
                                    function (err, hash) {
                                        if (err) {
                                            console.error(err);
                                            res.status(500).json({
                                                error: 'Ha ocurrido un error con el password',
                                            });
                                            return;
                                        }
                                        ...
                                        connection.query(
                                            ...
                                            function (error, results) {
                                                if (error) {
                                                    console.error(error);
                                                    res.status(500).json({
                                                        error: 'Ha ocurrido un error actualizando usuario',
                                                    });
                                                    return;
                                                } else {
                                                    res.status(200).json({
                ...
                res.status(404).json({
                    error: 'El PIN ha caducado o es incorrecto',
                });
            }
        }
    );
        ...
        if (email !== req.session.user.email) {
                ...
                connection.query(
                    ...
                    function (error, results, fields) {
                        if (error) {
                            console.error(error);
                            res.status(500).json({
                                error: 'Ha ocurrido un error al borrar el rol verificado del usuario',
                            });
                            return;
                        }
            ...
            connection.query(
                ...
                function (error, results, fields) {
                    ...
                    ejs.renderFile(
                        __dirname +
                            '/../config/nodemailer/templates/user-verificar_correo.ejs',
                        { username, link },
                        (error, data) => {
                            if (error) {
                                console.log(error);
...
router.get('/avatar/:id', (req, res) => {
    ...
    connection.query(
        'SELECT * FROM avatar_users WHERE user_id = ?',
        id,
        function (error, result) {
            if (error) {
                console.error(error);
                res.status(500).json({
                    error: 'Ha ocurrido un error consultando los avatar',
                });
                return;
            }
            if (result.length === 1) {
...
router.post('/avatar-update', (req, res) => {
    ...
    if (req.session.isLoggedIn && req.session.user.id === id) {
        connection.query(
            'UPDATE avatar_users SET avatar = ? WHERE user_id = ?',
            datosDeAvatar,
            function (error, result) {
                if (error) {
                    console.error(error);
                    res.status(500).json({
                        error: 'Ha ocurrido un error actualizando el avatar',
                    });
                    return;
                }
                if (result.changedRows === 1) {
                    ...
                    res.status(401).json({
                        message:
                            'solo usuarios registrados pueden cambiar su avatar',
                    });
                }
            }
        );
        ...
        res.status(401).json({
            message: 'Debes iniciar session para cambiar su avatar',
        });
    }
    ...
    connection.query(
        ...
        function (error, result) {
            if (error) {
                console.error(error);
                res.status(500).json({
                    message: 'Ha ocurrido un error actualizando el avatar',
                });
                return;
            } else {
                if (result.length >= 1) {
...
router.get('/email-verification/:token', (req, res) => {
    ...
    connection.query(query, [token, 'role'], (error, results) => {
        if (error) {
            res.status(500).json({
                message: 'Ha ocurrido un error al verificar el correo',
            });
            ...
            connection.query(query2, email, (error, results) => {
                if (error) {
                    res.status(500).json({
                        message:
                            'Ha ocurrido un error al consultar el correo en los usuarios',
                    });
                                            ...
                                            connection.query(
                                                ...
                                                function (
                                                    ...
                                                    if (error) {
                                                        console.error(error);
                                                        res.status(500).json({
                                                            error: 'Ha ocurrido un error al guardar el avatar en la BD',
                                                        });
                                                        return;
                                                    }
            ...
            res.status(403).json({
                message:
                    'El email ya ha sido verificado o debes volver a enviar un correo de verificacion',
            });
        }
    });
